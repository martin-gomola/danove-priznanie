...
import { ECB_RATE_2025, TAX_YEAR } from '@/lib/tax/constants';
...
function parseDividends(telo: Record<string, unknown>): ForeignDividends {
  ...
  for (const udaj of udajeList) {
    ...
    const amountUsd = (amountEur * ECB_RATE_2025).toFixed(2);
    const withheldTaxUsd = (withheldTaxEur * ECB_RATE_2025).toFixed(2);
